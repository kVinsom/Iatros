...
`internal/topology` associates normalized manifests with project boundaries, resolves only repository-confined workspace members, identifies direct local dependency targets, and propagates manifest diagnostics. `internal/analysis.LocalTopologyAnalyzer` composes the complete local pipeline. The `iatros topology` adapter maps this model into validated schema `0.3` text or JSON. `iatros analyze` uses its own schema `1.0` with unified findings and reads only bounded repository ignore/submodule control files beyond ordinary metadata.
