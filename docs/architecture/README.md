...
> [!IMPORTANT]
> **Status: target architecture with an initial CLI and core-contract implementation.** The root Go module, Cobra-based CLI adapter, local analysis placeholder, bounded filesystem discovery, broad filename-based technology detection, and versioned private project and workflow contracts are implemented internally. Discovery and detection are not yet connected to CLI reports; every broader runtime, API, SDK, plugin, workflow, and security capability remains target behavior unless explicitly marked otherwise.
...
The repository contains the canonical directory scaffold, project metadata, branding, documentation, one root Go module, a tested `iatros analyze` contract stub, a bounded filesystem inventory service, a broad internal technology detector, and stable private contracts for project topology and workflow results. Discovery, detection, and the new domain contracts are not yet integrated into the user-visible command result. The repository does not yet contain readiness rules, public schemas, deployment configuration, CI workflows, or releases.
...
| --- | --- | --- |
| Canonical project model | `internal/project` | Implemented private provider-neutral `1.0` representation of projects, environments, services, dependencies, and configuration. |
| Lifecycle-stage result | `internal/workflow` | Implemented private `1.0` envelope with canonical stages, terminal outcomes, identities, timestamps, and diagnostics. |
| Private schema compatibility | `internal/schema` | Implemented canonical `major.minor` parsing and reader compatibility rules. |
